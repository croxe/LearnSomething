...
JVM是java虚拟机，可以运行字节码，在主流平台上都有实现，并且同样的字节码会跑出同样的结果。

什么是字节码?采用字节码的好处是什么?

字节码是JVM可执行语言，使用.class文件包装，JVM执行字节码后会把字节码转换成机器码来执行。字节码不面向特定处理器，只面向JVM。Java 语言通过字节码的方式，在一定程度上解决了传统解释型语言执行效率低的问题，同时又保留了解释型语言可移植的特点。Java程序无需重新编译即可使用字节码在不同操作系统上运行。

Java 程序从源代码到运行一般有三步：

![image info](./Picture/JavaBasic_3.png)
...
在c里面字符串是没有长度属性的，长度都是用函数来实现的。Java里面一切都是对象，是对象的话，字符串肯定就有长度，即然有长度，编译器就可以确定要输出的字符个数，当然也就没有必要去浪费那1字节的空间用以标明字符串的结束了。比如，数组对象里有一个属性length,就是数组的长度，String类里面有方法length()可以确定字符串的长度，因此对于输出函数来说，有直接的大小可以判断字符串的边界，编译器就没必要再去浪费一个空间标识字符串的结束。
...
> ![image info](./Picture/JavaBasic_4.jfif)
...
程序、类、变量、方法等取名字，就是标识符。特殊的标识符就是关键字。"for"
...
continue ：指跳出当前的这一次循环，继续**下一次**循环。
...
A B C D 26 个字母可以做到互换，但是 E K V T 有更好的可读性。
...
![image info](./Picture/JavaBasic_5.jfif)
...
![image info](./Picture/JavaBasic_6.jfif)
...
![image info](./Picture/JavaBasic_7.jfif)

交换之后：

![image info](./Picture/JavaBasic_8.jfif)
...
![image info](./Picture/JavaBasic_9.jfif)
...
![image info](./Picture/JavaBasic_10.jfif)
